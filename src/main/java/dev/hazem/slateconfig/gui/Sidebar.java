...
public class Sidebar extends ScrollableWidget {
    ...
    @Override
    protected void init() {
        this.scrollbar.init(x + width - 5, y, height, totalExtent);
    }
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        for (PageNode page : metadata.getPages()) {
            boolean isSelected = page == metadata.currentPage;
            ...
            Box box = new Box(x+paddingX/2, y-(paddingY/2), width-paddingX - 4, fontHeight+paddingY);

            if (isSelected) box.render(context, 4, 0x10FFFFFF);
            if (box.isHovered(scissorBox, mouseX, mouseY) && !scrollbar.isDragging()) {
